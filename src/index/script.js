...
const loader = () => {
    ...
    const jobsFetch = async () => {
        try {
            const response = await fetch("https://remotive.com/api/remote-jobs?limit=150")
   ...
   const buildCard = (obj) => {
    ...
    mainDiv.setAttribute("class","card border border-4 border border-warning shadow p-3 mb-5 bg-body rounded rounded")
...
const positionsByCategory_fetch = async (position) => {
    try {
        ...
        const response = await fetch(`https://remotive.com/api/remote-jobs?category=${position}`)
        const data = await response.json()
